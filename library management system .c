...
int s;
char findbook;
char username[10]={"cproject"};
char password[10]={"cproject"};
...
void deletebooks()    //function that delete items from file fp
{
    ...
    if(flg==1)
    {
        ...
        if(getch()=='y')
        {
            ...
            fclose(fp);
            remove("Bibek.txt");
            rename("test.txt","Bibek.txt"); //copy all item from temporary file to fp except that
            gotoxy(10,10);
...
void searchbooks()
{
    ...
    gotoxy( 15,20);
    printf("Enter Your Choice");
    fp=fopen("Bibek.txt","rb+"); //open file for reading propose
    ...
    switch(getch())
    {
        case '1':
        {
            ...
            if(flg==0)  //checks whether condition enters inside loop or not
            {
                gotoxy(20,8);
...
void issuebooks(void)  //function that issue books from library
{
    ...
    switch(getch())
    {
        case '1':  //issue book
        {
            ...
            printf("Enter the Book Id:");
            scanf("%d",&id);
            fp=fopen("Bibek.txt","r+");
            fs=fopen("Issue.txt","a+");
            if(checkid(id)==0&&a.quantity!=0) //issues those which are present in library
            {
                gotoxy(10,8);
                printf("The book record is available");
                gotoxy(10,9);
                ...
                scanf("%s",a.stname);
                SYSTEMTIME stime;  //for current date
                GetSystemTime(&stime);
                a.issued.dd=stime.wDay;
                a.issued.mm=stime.wMonth;
                a.issued.yy=stime.wYear;
                gotoxy(10,12);
            ...
            {
                gotoxy(10,11);
                printf("The Book is not available for issue");
                gotoxy(10,13);
                fclose(fp);
                returnfunc();
            }
